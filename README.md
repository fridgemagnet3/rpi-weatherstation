...
## MQTT publisher and Home Assistant Integration

The [mqtt/weather_mqtt_publisher.py](mqtt/weather_mqtt_publisher.py) script listens for the weather UDP broadcast packets, then publishes them to an MQTT broker. The script also publishes HA [MQTT auto-discovery messages](https://www.home-assistant.io/integrations/mqtt#mqtt-discovery) so they should autmatically appear as sensors within the system. 
