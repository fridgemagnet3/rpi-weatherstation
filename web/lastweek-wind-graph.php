...
$query = "select Date,Max,Average,MaxAvg from wind_speed where date<='" . 
...
while ( $row = $result->fetch_assoc())
{
    ...
    $TempChart->addPoints($row['Average'],"Avg") ;
    $TempChart->addPoints($row['MaxAvg'],"Max Avg") ;
    $TempChart->addPoints(date("D",strtotime($row['Date'])),"Labels") ;
}
$TempChart->setAxisName(0,"Wind Speed(mph)");
...
$TempGraph->drawText(150,35,"Wind Speed",array("FontSize"=>20,"Align"=>TEXT_ALIGN_BOTTOMMIDDLE));
